...
describe('The /users/signin route', () => {
  ...
  it('Should show an error with empty request', async () => {
    ...
    expect(res.body.error).to.contain("Cannot read property 'trim' of undefined");
  });
  ...
  it('Should send a json structure after succesful signin', async () => {
    ...
    const res = await helper.loginUser(agent, helper.validUser.username, helper.validUser.password);
    // eslint-disable-next-line no-unused-expressions
